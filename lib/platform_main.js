 ...
 * framework from simpler components.
 *
...
exports.newInstance = function($, spec, cb) {
    try {
         ...
         *
         * @memberof! module:caf_platform/platform_main#
        ...
        if (spec.env['debugger'] && (spec.env['debugger'] !== 'false')) {
            // Debugger activated by setting NODE_OPTIONS before `node` starts
            $._.$.log && $._.$.log.debug('Debugger active NODE_OPTIONS=' +
                                         process.env['NODE_OPTIONS']);
        } else {
            /* Too late by now, need to set it before launching 'node',
        ...
        const notifyF = function(err, res) {
            if (err) {
                ...
                $._.$.log && $._.$.log.trace('Check OK:' + JSON.stringify(res));
            }
            if (pendingShutdown) {
