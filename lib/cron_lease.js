        ...
        var renewF = function() {
            ...
            var cb0 = function(err, gone) {
                if (err) {
                     ...
                     * and therefore, and we do not need to explicitly kill all
                     *  children. (Comment1)
                     */
                } else {
                    async.eachSeries(gone, function(id, cb1) {
                                         $._.$.registry
                                             .__ca_deleteChild__(null, id,
                                                                 cb1);
                                     }, function(err) {
                                         if (err) {
                                             var msg = 'cron_lease: Cannot ' +
                                                 ' delete child ';
                                             $._.$.log && $._.$.log.error(msg +
                                                myUtils.errToPrettyStr(err));
                                         }
                                         /*
